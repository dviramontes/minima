...
pub fn main() !void {
    ...
    if (parser.args.len == 0) {
        ...
        if (csv.fileExists(csv_path)) {
            ...
            var loaded_habits = csv.loadHabitsFromCsv(allocator, csv_path) catch |err| {
            ...
            // Render loaded habits
            try tui.render(loaded_habits.items);
        } else {
            // File doesn't exist, use example habits
        ...
        for (parser.args) |habit| {
            // TODO: create a new date per habit
